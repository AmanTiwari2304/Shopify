...
export default function LoginPage() {
    ...
    const onAdminDetailbtn = async () => {
        try {
            setLoading(true);
            const response = await axios.post("/api/alreadyAnAdmin", admin)
            console.log("Admin Detail success",response.data)
            toast.success("Admin Detail success")
            // redirect to dashboard including adminEmail as query param
            router.push(`/dashboard?adminEmail=${encodeURIComponent(admin.email)}`);
        } catch (error: any) {
            console.log("Admin Detail Failed", error.message);
    ...
    return(
        ...
        <div className="flex min-h-full flex-col justify-center px-6 py-12 lg:px-8">
            <div className="sm:mx-auto sm:w-full sm:max-w-sm">
                ...
                {loading ? "Processing" : "Fill Admin Detail"}</h2>
            </div>
            ...
            <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
                ...
                <div>
                    <Link href="/admin" className="flex w-full justify-center rounded-md bg-indigo-600 px-3 py-1.5 text-sm/6 font-semibold text-white shadow-xs hover:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600 dark:bg-indigo-500 dark:shadow-none dark:hover:bg-indigo-400 dark:focus-visible:outline-indigo-500">Visit Be Admin Page</Link>
                </div>
                </form>
